...
- **Laravel Cloud:** SSO, CloudPCR (`pdo_sqlsrv` available), Billing, CAD, Fire, Reporting, CheckSheets, Drug-Tracking, Crew-Scheduling, account, wiki, Transport-Portal, Public_Website, University (confirmed 2026-08-18; no `vapor.yml`, prod DB verified against the Cloud env). Pushing the deploy branch **is** the deploy (push-to-deploy) — a push to `main` on these repos triggers a production build.
- **Vapor:** HR, Truck-Checks, Fleet-Management. (Crew-Scheduling moved to Laravel Cloud at its 2026-08-05 React cutover; University turned out to be on Laravel Cloud all along, confirmed 2026-08-18.) `pdo_sqlsrv` NOT available — use API calls, not direct SQL Server access. **HR, Truck-Checks, and Fleet-Management run out of their Vapor *staging* environments** (`*-staging.unified-apps.com`) — staging IS live; do not run `vapor deploy production` for them, and treat `vapor deploy staging` as a real production deploy requiring explicit go-ahead.
...
- **SSO is authoritative for trial status** (`companies.status === 'trial'`; endpoint `GET /api/internal/companies/trials`, `CORE_APP_API_KEY` auth). Never identify trials from a downstream app's local `is_live` flag — it's also false for prospects/suspended, and stale in CloudPCR. All trial machinery (seed commands AND the `trial.purge_data` webhook handler) must fail closed if SSO is unreachable — the sso-client package verifies a purge target against this endpoint (trials list + `purge_pending` intent ids) before dispatching the app's purger.
...
- **Config fallback URLs must never point at `.test` hosts.** A production process can fall through to a config fallback (missing env var, stale env snapshot) and a `.test` fallback then fails DNS instead of degrading safely — 767 CloudPCR claim-status callbacks died this way over five weeks (Billing, 2026-07/08). Cross-app URL fallbacks default to the production URL; local dev sets the `.test` value in `.env`.
- **A staging environment that shares production's database must not consume production's queues or run a scheduler.** Billing staging ran a bare `queue:work` background process that consumed the shared `jobs` table with stale code and stale env, silently failing prod jobs while surviving every production deploy and `queue:restart` (Pure Hearts stale-status incident, 2026-08-27; the scheduler variant was UNI-300). When worker behavior looks haunted, probe which HOST serves each queue (Billing's `billing:queue-watchdog` / `queue_probes` pattern) before trusting deploy-side fixes.
- **The session cannot enforce "use this once".** Laravel reads a session at the start of a request and writes it back at the end, so `Session::pull()` is a read now and a write hundreds of milliseconds later — two overlapping requests both see the value intact and both act on it. Anything single-use (an OAuth authorization code, a rotating refresh token, a one-shot action token) has to be claimed with an atomic conditional write in the SHARED cache (`Cache::add()`, `Cache::lock()`), which is also the only form that holds across the several instances Laravel Cloud and Vapor run. The sso-client `SsoSingleFlight` class is the reference (UNI-438, UNI-455). Fail such a guard OPEN — a cache outage must degrade to the old behaviour, never lock everyone out.
