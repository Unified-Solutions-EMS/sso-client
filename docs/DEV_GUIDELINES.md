...
- **Laravel Cloud:** SSO, CloudPCR (`pdo_sqlsrv` available), Billing, CAD, Fire, Reporting, CheckSheets, Drug-Tracking, account, wiki, Transport-Portal, Public_Website. Pushing the deploy branch **is** the deploy (push-to-deploy) — a push to `main` on these repos triggers a production build.
- **Vapor:** HR, Truck-Checks, Fleet-Management, Crew-Scheduling, University. `pdo_sqlsrv` NOT available — use API calls, not direct SQL Server access. **HR, Truck-Checks, and Fleet-Management run out of their Vapor *staging* environments** (`*-staging.unified-apps.com`) — staging IS live; do not run `vapor deploy production` for them, and treat `vapor deploy staging` as a real production deploy requiring explicit go-ahead.
